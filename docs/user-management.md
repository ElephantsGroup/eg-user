...
When you start using eg-user you will probably find that you need to create, update and delete accounts of other users.
In order to do that eg-user provides CRUD interface.
...
```
if (Yii::$app->session->has(\elephantsGroup\user\controllers\AdminController::ORIGINAL_USER_SESSION_KEY))
    echo Html::a(
...
echo Nav::widget([
    ...
    'items' => [
        Yii::$app->session->has(\elephantsGroup\user\controllers\AdminController::ORIGINAL_USER_SESSION_KEY) ?
